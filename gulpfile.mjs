...
async function processFile(pfx) {
    const contents = yaml.parse(await readFile(join('./sources', `${pfx}.yaml`)).then((b) => b.toString()));
...
async function config() {
    ...
    const contents = yaml.parse(await readFile(join('./sources', `config.yaml`)).then((b) => b.toString()));
...
async function translations() {
    ...
    const files = await readdir(join('./sources', 'translations'));
    const keys = {};
    const backend = {};

    for (const file of files) {
        const fullPath = join('./sources', 'translations', file);
...
export default async function() {
    ...
    const outputs = (await readdir('./sources'))
